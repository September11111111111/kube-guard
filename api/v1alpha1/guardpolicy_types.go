...
type GuardPolicySpec struct {
	...
	Threshold            string    `json:"threshold"` // 例如 ">0"
	TargetRef            TargetRef `json:"targetRef"`
	EvaluateEverySeconds int32     `json:"evaluateEverySeconds,omitempty"`
	CooldownSeconds      int32     `json:"cooldownSeconds"`
}
type TargetRef struct {
	Namespace string `json:"namespace"`
	Kind      string `json:"kind"` // 先只支持 Deployment
...
type GuardPolicyStatus struct {
	...
	// conditions represent the current state of the GuardPolicy resource.
...
type GuardPolicy struct {
	...
	// spec defines the desired state of GuardPolicy
	// +required
	Spec GuardPolicySpec `json:"spec"`

	// status defines the observed state of GuardPolicy
	// +optional
	Status GuardPolicyStatus `json:"status,omitzero"`
}
